...
### ローカルでの自動テスト（push 前にローカルで実行）

セキュリティ上の理由でリモート CI を使いたくない場合は、push 前にローカルでテストを実行して失敗を防ぐのが最も単純で安全です。本リポジトリには `.githooks/pre-push` のテンプレートを用意しています。

有効化手順（各開発者のローカルで1回実行）:

```bash
# リポジトリルートで1回だけ実行
git config core.hooksPath .githooks

# 確認: pre-push が使われるようになる
git config --get core.hooksPath
```

動作内容:
- `git push` を実行すると `poetry install`（可能なら）と `poetry run pytest` が走ります。
- テストに失敗すると push は中止されます（`--no-verify` を使えば強制 push できますが推奨しません）。

注意点:
- フックはローカルに依存するため、CI 保証の代替ではありませんが、機密リポジトリやオフライン環境では有用です。
- テストが重い場合は、フック内で速いスモークテストのみ実行するようカスタマイズしてください（例: `pytest tests/unit -q`）。
